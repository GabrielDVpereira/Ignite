...
export function useUsers(page: number, options: UseQueryOptions<GetUsersResponse, Error>) {
  ...
  return useQuery<GetUsersResponse, Error>(['users', page] /** passing an array because we need different cache for each page */, () => getUsers(page) , { // we set a name for the query becase that's the name of the caching later, then a function that will return the data that will be stored
